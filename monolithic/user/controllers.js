...
router.post('/', (req, res, next) => {
  ...
  Joi.validate(body, schema, (err, value) => {
    if (err) {
      return res.status(401).json({ status: 'ERROR', msg: err.details[0].message })
    } else {
...
router.post('/activate/:code', (req, res, next) => {
  ...
  queries().activate(code, (err, response) => {
    if (err) next({ err })
      ...
      if (response.rowCount > 0) { res.json({ status: 'OK', msg: 'Cadastro confirmado.' }) } else { res.json({ status: 'ERROR', msg: 'Cadastro não encontrado.' }) }
    }
  })
})
  ...
  queries().update(id, password, (err, response) => {
    if (err) next({ err })
      ...
      if (response.rowCount > 0) {
        ...
        res.json({ status: 'ERROR', msg: 'Cadastro não encontrado.' })
      }
    }
  })
})
...
router.post('/:id/recoveryPassword', (req, res, next) => {
  const id = req.params.id
  queries().findUserByClientId(id, (err, response) => {
    if (response.rowCount > 0) {
      const user = response.rows[0]
      const newPassword = randomstring.generate(15)

      queries().recovery(id, newPassword, (err, response) => {
        if (err) next({ err })
        else {
          email().sendEmail({
